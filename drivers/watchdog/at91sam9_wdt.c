...
static int at91_wdt_start(struct udevice *dev, u64 timeout_s, ulong flags)
{
	 ...
	 * 4096 / 256 = 16 seconds.
	 */
	priv->regval &= ~AT91_WDT_MR_WDV;
	priv->regval &= ~AT91_WDT_MR_WDD;
	priv->regval |= AT91_WDT_MR_SET_WDV(timeout);
	priv->regval |= AT91_WDT_MR_SET_WDD(0xfff);
	priv->regval |= AT91_WDT_MR_WDDBGHLT;
